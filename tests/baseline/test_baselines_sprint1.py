...
def test_linear_threshold(X_train, T_train, X_test, T_test):
    ...
    model = LinearThresholdHeuristic(threshold=0.5, k_steps=3, normalize=True)
...
def test_markov_chain(X_train, T_train, X_test, T_test):
    ...
    model = MarkovChainModel(
        n_states=5, collapse_quantile=0.2, normalize=True, random_state=42
    )
    model.fit(X_train, T_train)
...
def main():
    ...
    logger.info(
        f"Test: {len(T_test)} samples, T_event range [{T_test.min():.1f}, {T_test.max():.1f}]"
    )
    ...
    try:
        results["linear_threshold"] = test_linear_threshold(
