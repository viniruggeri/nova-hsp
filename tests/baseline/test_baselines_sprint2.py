...
def synthetic_data():
    ...
    T_train = 50 - 10 * (feature_sum - feature_sum.min()) / (
        feature_sum.max() - feature_sum.min()
    )
    ...
    T_test = 50 - 10 * (feature_sum_test - feature_sum_test.min()) / (
        feature_sum_test.max() - feature_sum_test.min()
    )
...
def test_temporal_lstm_basic(synthetic_data):
    ...
    model = TemporalClassifier(
        input_dim=n_features, hidden_dim=32, num_layers=1, dropout=0.1, rnn_type="lstm"
    )
    model.fit(X_train, T_train, epochs=20, batch_size=2, lr=1e-3)
...
def test_temporal_gru(synthetic_data):
    ...
    model = TemporalClassifier(
        input_dim=n_features, hidden_dim=32, num_layers=1, rnn_type="gru"
    )
