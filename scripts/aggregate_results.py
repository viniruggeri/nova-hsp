...
def main():
    ...
    parser.add_argument(
        '--metrics',
        nargs='+',
        default=['MAE', 'RMSE', 'c_index', 'mean_degradation'],
    ...
    aggregator.load_results_from_dir(
        ...
        pattern='**/integrated_results.csv'
    )
    ...
    if len(aggregator.results) >= 2:
        ...
        for i, exp1 in enumerate(exp_names):
            for exp2 in exp_names[i+1:]:
                ...
                pairwise = aggregator.pairwise_comparison(exp1, exp2, metric=args.metrics[0])
    ...
    if not ranked.empty:
        ...
        for idx, row in top5.iterrows():
            logger.info(f"  {int(row['rank'])}. {row['model_name']} (score: {row['composite_score']:.4f})")
