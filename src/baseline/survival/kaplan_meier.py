...
class KaplanMeierModel(BaseEstimator):
    ...
    def fit(self, durations: np.ndarray, events: np.ndarray = None):
        ...
        if len(durations) != len(events):
            raise ValueError(
                f"durations and events must have same length: {len(durations)} vs {len(events)}"
            )
        ...
        logger.info(
            f"Kaplan-Meier fitted on {len(durations)} samples, "
    ...
    def predict_median_time(self) -> float:
        ...
        try:
            ...
            if np.isnan(median):
                ...
                logger.warning(
                    "Median survival time not reached, using mean as fallback"
                )
