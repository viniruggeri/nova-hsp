...
class MarkovChainModel(BaseEstimator):
    ...
    def fit(self, X: np.ndarray, T: np.ndarray):
        ...
        if X.ndim != 3:
            raise ValueError(
                f"X must be 3D (n_samples, seq_len, n_features), got shape {X.shape}"
            )
        ...
        # Cluster states
        logger.info(
            f"Clustering {len(X_flat)} timesteps into {self.n_states} states..."
        )
        state_labels = self.kmeans.fit_predict(X_flat)

        # Reshape back to sequences
        state_sequences = state_labels.reshape(
            n_samples, seq_len
        )  # (n_samples, seq_len)
        ...
        logger.info(
            f"Markov Chain fitted: {self.n_states} states, "
    ...
    def _identify_collapse_states(
        self, state_sequences: np.ndarray, T: np.ndarray
    ) -> set:
        """
    ...
    def predict_time_to_collapse(
        self, X: np.ndarray, max_steps: int = 100, n_simulations: int = 50
    ) -> np.ndarray:
        """
