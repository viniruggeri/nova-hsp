...
class PaperVisualizer:
    ...
    def plot_model_comparison(
        ...
        for idx, metric in enumerate(metrics):
            ...
            best_idx = (
                data[metric].argmin()
                if any(err in metric.lower() for err in ["mae", "rmse", "error"])
                else data[metric].argmax()
            )
    ...
    def plot_robustness_degradation(
        ...
        for idx, pert_type in enumerate(pert_types):
            ...
            pert_data = robustness_results[
                robustness_results["perturbation_type"] == pert_type
            ]

            for model_idx, model in enumerate(models):
                model_data = pert_data[pert_data["model_name"] == model].sort_values(
                    "intensity"
                )
        ...
        if normalize_cols:
            for metric in metrics:
                ...
                if max_val - min_val > 0:
                    ...
                    is_error = any(
                        err in metric.lower() for err in ["mae", "rmse", "error"]
                    )

                    if is_error:
                        # Lower is better: invert so higher values = better
                        pivot_data[metric] = 1 - (values - min_val) / (
                            max_val - min_val
                        )
                    else:
    ...
    def generate_all_plots(
        ...
        if len(metrics) >= 2:
            ...
            self.plot_pareto_front(
                ...
                x_ascending=False,  # basin_contraction_corr: higher is better
                y_ascending=False,  # lead_time_norm: higher is better
            )
