...
class IntegratedEvaluator:
    ...
    def evaluate_performance(self) -> pd.DataFrame:
        ...
        self._performance_results = self.evaluator.evaluate_all(
            X=self.X, T_true=self.T, events=self.E
        )
        ...
        for model_name, model in self.models.items():
            try:
                ...
                for pert_type, pert_data in results.items():
                    for intensity, degradation in zip(
                        pert_data["intensities"], pert_data["degradations"]
                    ):
                        robustness_results.append(
                            {
                                ...
                                "perturbed_error": pert_data["metric_clean"]
                                * (1 + degradation / 100),
                            }
                        )
    ...
    def evaluate_all(
        self, include_cv: bool = False, include_advanced: bool = False
    ...
    def _create_integrated_summary(
        self, performance_df: pd.DataFrame, robustness_df: pd.DataFrame
    ) -> pd.DataFrame:
        """
        ...
        robustness_summary = (
            robustness_df.groupby("model_name")
            .agg({"degradation_percent": ["mean", "std"], "perturbed_error": "mean"})
            .reset_index()
        )
        ...
        auc_d_pivot = auc_d_df.pivot(
            index="model_name", columns="perturbation_type", values="auc_d"
        ).reset_index()
        ...
        if "MAE" in integrated.columns:
            ...
            deg_norm = (
                integrated["mean_degradation"] - integrated["mean_degradation"].min()
            ) / (
                integrated["mean_degradation"].max()
                - integrated["mean_degradation"].min()
                + 1e-10
            )
    ...
    def compare_models(self) -> pd.DataFrame:
        ...
        if self._robustness_results is not None:
            ...
            rob_summary = self._robustness_results.groupby("model_name")[
                "degradation_percent"
            ].mean()
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> str:
        ...
        perf_summary = perf_df[
            ["model_name", "MAE", "RMSE", "R2", "Lead_Time"]
        ...
        report.append("-" * 80)
        rob_summary = (
            rob_df.groupby(["model_name", "perturbation_type"])
            .agg({"degradation_percent": ["mean", "std"]})
            .to_string()
        )
    ...
    def save_results(self, output_dir: Path):
        ...
        if self._performance_results is not None:
            self._performance_results.to_csv(
                output_dir / "performance_results.csv", index=False
            )
            logger.info(
                f"Saved performance results to {output_dir / 'performance_results.csv'}"
            )

        if self._robustness_results is not None:
            self._robustness_results.to_csv(
                output_dir / "robustness_results.csv", index=False
            )
            logger.info(
                f"Saved robustness results to {output_dir / 'robustness_results.csv'}"
            )

        if self._integrated_results is not None:
            self._integrated_results.to_csv(
                output_dir / "integrated_results.csv", index=False
            )
            logger.info(
                f"Saved integrated results to {output_dir / 'integrated_results.csv'}"
            )
