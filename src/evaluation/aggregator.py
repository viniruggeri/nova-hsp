...
class ResultsAggregator:
    ...
    def load_results_from_dir(
        self, results_dir: Path, pattern: str = "**/integrated_results.csv"
    ):
        """
    ...
    def aggregate_by_model(self, metrics: Optional[List[str]] = None) -> pd.DataFrame:
        """
        ...
        # Aggregate by model
        agg_funcs = {
            metric: ["mean", "std", "median", "min", "max"] for metric in metrics
        }
    ...
    def compare_across_worlds(
        self, metric: str = "MAE", alpha: float = 0.05
    ) -> pd.DataFrame:
        """
    ...
    def pairwise_comparison(
        self, exp1: str, exp2: str, metric: str = "MAE", test: str = "mannwhitneyu"
    ) -> pd.DataFrame:
        """
            ...
            try:
                ...
                if test == "mannwhitneyu":
                    ...
                    if len(values1) == len(values2):
                        ...
                        logger.warning(
                            f"Wilcoxon requires equal sample sizes, skipping {model}"
                        )
                        continue
                elif test == "ttest":
    ...
    def rank_models(
        ...
        if ascending is None:
            ...
            for metric in metrics:
                ...
                is_error = any(
                    err in metric.lower()
                    for err in ["mae", "rmse", "mse", "error", "brier", "ibs"]
                )
        ...
        for metric, asc in zip(metrics, ascending):
            ...
            if max_val - min_val > 0:
                if asc:
                    ...
                    normalized[f"{metric}_norm"] = (values - min_val) / (
                        max_val - min_val
                    )
                else:
                    # Higher is better: normalize so 1 = best, 0 = worst
                    normalized[f"{metric}_norm"] = 1 - (values - min_val) / (
                        max_val - min_val
                    )
            else:
            ...
            for col in export_df.columns:
                ...
                values = aggregated[
                    [
                        ...
                        if col.lower() in c.lower() and "_mean" in c
                    ]
                ]
                if not values.empty:
                    if is_error:
