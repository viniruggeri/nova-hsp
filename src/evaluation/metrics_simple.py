...
def compute_lead_time(
    T_event: np.ndarray, T_alert: np.ndarray, T_start: float = 0.0
) -> np.ndarray:
    """
...
class BaselineEvaluator:
    ...
    def evaluate_model(
        self, model, X_test: np.ndarray, T_test: np.ndarray, model_name: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"{model_name}: Lead Time = {stats['mean']:.3f} ± {stats['std']:.3f}, "
