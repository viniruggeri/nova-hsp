...
class UnifiedBaselineEvaluator:
    ...
    def _detect_model_types(self):
        ...
        for name, model in self.models.items():
            ...
            # Survival models
            if (
                "KaplanMeier" in class_name
                or "CoxPH" in class_name
                or "AFT" in class_name
            ):
                self.model_types[name] = "survival"
            # Heuristic models
            elif (
                "Threshold" in class_name
                or "Warning" in class_name
                or "EWS" in class_name
            ):
            ...
            # Deep learning models
                ...
                "Deep" in class_name
                or "LSTM" in class_name
                or "Temporal" in class_name
                or "Hazard" in class_name
            ):
                ...
                if hasattr(model, "predict_median_time"):
                    ...
                    logger.warning(
                        f"Could not detect type for model {name}, defaulting to 'unknown'"
                    )
            ...
            if model_type == "survival":
                ...
                if hasattr(model, "predict_median_time"):
                    ...
                    raise AttributeError(
                        f"Survival model {model_name} has no predict_time method"
                    )
                ...
                if hasattr(model, "predict_time_to_collapse"):
                    ...
                    if X.ndim == 3:  # Batch
                        pred = np.array(
                            [model.predict_alert_time(X[i]) for i in range(len(X))]
                        )
                    else:  # Single sample
                        pred = np.array([model.predict_alert_time(X)])
                else:
                    raise AttributeError(
                        f"Heuristic model {model_name} has no predict method"
                    )
    ...
    def compute_metrics(
        ...
        if not valid_mask.all():
            logger.warning(
                f"Found {(~valid_mask).sum()} invalid predictions, excluding from metrics"
            )
            ...
            for model_name, model in self.models.items():
                try:
                    ...
                    if hasattr(model, "fit"):
                        ...
                        if (
                            self.model_types[model_name] == "survival"
                            and "KaplanMeier" in model.__class__.__name__
                        ):
                            model.fit(
                                T_train,
                                (
                                    ...
                                    else np.ones_like(T_train)
                                ),
                            )
                        elif (
                            self.model_types[model_name] == "survival"
                            and events_train is not None
                        ):
                        ...
                        if verbose:
                            logger.warning(
                                f"{model_name} has no fit method, using existing model"
                            )

                    # Evaluate
                    result = self.evaluate_single(
                        model_name, X_test, T_test, events=events_test
                    )
    ...
    def compare_models(
        self, results_df: pd.DataFrame, metric: str = "MAE", test: str = "wilcoxon"
    ) -> pd.DataFrame:
        """
        ...
        for i, model1 in enumerate(models):
            for j, model2 in enumerate(models):
                ...
                try:
                    ...
                    logger.warning(
                        f"Statistical test failed for {model1} vs {model2}: {e}"
                    )
                    p_values[i, j] = np.nan
    ...
    def summary_statistics(
        self, cv_results: pd.DataFrame, metrics: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
    ...
    def evaluate_with_advanced_metrics(
        ...
        try:
            ...
            logger.error(
                "scikit-survival not available. Install with: pip install scikit-survival"
            )
            return pd.DataFrame()
