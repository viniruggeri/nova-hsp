...
class TemporalDataSplitter:
    ...
    def _load_world_data(
        self,
    ) -> Tuple[List[str], List[int], List[float], pd.DataFrame]:
        ...
        if self.horizon is None:
            if self.world_name == "sir_graph":
                ...
                if (data_path / "labels_horizon_15_train.csv").exists():
                    self.horizon = 15
                    logger.info(
                        "Auto-detected: Using horizon_15 (better class balance)"
                    )
                else:
                    self.horizon = 20
                    logger.info(
                        "Auto-detected: Using horizon_20 (horizon_15 not available)"
                    )
            else:
        ...
        if all_runs_file.exists():
            ...
            run_metadata = (
                all_runs.groupby("seed")
                .agg({"time_to_event": "first", "run_id": "first"})
                .reset_index()
            )
            ...
            run_metadata = pd.concat(all_meta, ignore_index=True).drop_duplicates(
                subset=["seed"]
            )
        ...
        for split in ["train", "val", "test"]:
            ...
            if labels_file.exists():
                lbl = pd.read_csv(labels_file)[
                    ["seed", f"label_horizon_{self.horizon}"]
                ]
                lbl = lbl.rename(
                    columns={f"label_horizon_{self.horizon}": "label_horizon"}
                )
                labels_data.append(lbl)

        if labels_data:
            all_labels = pd.concat(labels_data, ignore_index=True).drop_duplicates(
                subset=["seed"]
            )
            run_metadata = run_metadata.merge(all_labels, on="seed", how="left")
            run_metadata["label_horizon"] = (
                run_metadata["label_horizon"].fillna(0).astype(int)
            )
        else:
        ...
        logger.info(
            f"Using horizon_{self.horizon} | Label distribution: {run_metadata['label_horizon'].value_counts().to_dict()}"
        )

        return seeds, T_events, run_metadata

    def _temporal_split(
        self, n_samples: int, labels: np.ndarray
        ...
        if min_samples >= 2:
            ...
            n_temp = len(temp_idx)
            val_size = int(
                n_temp * (self.val_ratio / (self.val_ratio + self.test_ratio))
            )
            ...
            if temp_labels.sum() >= 2 or (len(temp_labels) - temp_labels.sum()) >= 2:
                ...
                val_local_idx, test_local_idx = next(
                    sss2.split(np.arange(n_temp), temp_labels)
                )
            else:
            ...
            logger.warning(
                f"Class imbalance detected (min samples={min_samples}). Using temporal split."
            )
        ...
        logger.info(
            f"  Train label dist: {dict(zip(*np.unique(labels[train_idx], return_counts=True)))}"
        )
        logger.info(
            f"  Val label dist:   {dict(zip(*np.unique(labels[val_idx], return_counts=True)))}"
        )
        logger.info(
            f"  Test label dist:  {dict(zip(*np.unique(labels[test_idx], return_counts=True)))}"
        )
    ...
    def _load_observations(
        ...
        logger.info(
            f"Loaded {len(features_list)} observations: X.shape={X.shape}, y.shape={y.shape}, T.shape={T.shape}"
        )
        ...
        logger.info(
            f"Loaded observations: X.shape={X.shape}, y.shape={y.shape}, T.shape={T.shape}"
        )
